...
variable "vzen_count" {
  type        = number
  description = "Number of VZENs to create. 1 VZEN by default in case of standalone deployment type."
  default     = 1
}
...
variable "byo_igw_id" {
  type        = string
  description = "User provided existing AWS Internet Gateway name. This must be populated if brownfield_deployment variable is true"
  default     = ""
}
